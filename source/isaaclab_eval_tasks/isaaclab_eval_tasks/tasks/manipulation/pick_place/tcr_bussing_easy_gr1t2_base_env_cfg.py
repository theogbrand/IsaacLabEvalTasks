...
##
@configclass
class TcrBussingEasySceneCfg(InteractiveSceneCfg):

    # Table (represents sink surface)
    table = AssetBaseCfg(
        prim_path="/World/envs/env_.*/Table",
        init_state=AssetBaseCfg.InitialStateCfg(pos=[0.0, 0.55, 0.0], rot=[1.0, 0.0, 0.0, 0.0]),
        spawn=UsdFileCfg(
            usd_path=f"{ISAACLAB_NUCLEUS_DIR}/Props/Mounts/SeatingPlane/table_instanceable.usd",
            scale=(1.0, 1.0, 1.0),
            rigid_props=sim_utils.RigidBodyPropertiesCfg(),
        ),
    )

    # Trash item 1 (using factory nut as placeholder)
    trash_item_1 = RigidObjectCfg(
        prim_path="{ENV_REGEX_NS}/TrashItem1",
        init_state=RigidObjectCfg.InitialStateCfg(pos=[-0.05, 0.50, 0.986], rot=[1.0, 0, 0, 0]),
        spawn=UsdFileCfg(
            usd_path=f"{ISAACLAB_NUCLEUS_DIR}/Props/Mimic/nutpouring_task_demo/factory_nut.usd",
            scale=(1.0, 1.0, 1.0),
            rigid_props=sim_utils.RigidBodyPropertiesCfg(),
        ),
    )

    # Trash item 2 (using factory nut as placeholder)
    trash_item_2 = RigidObjectCfg(
        prim_path="{ENV_REGEX_NS}/TrashItem2",
        init_state=RigidObjectCfg.InitialStateCfg(pos=[0.05, 0.50, 0.986], rot=[1.0, 0, 0, 0]),
        spawn=UsdFileCfg(
            usd_path=f"{ISAACLAB_NUCLEUS_DIR}/Props/Mimic/nutpouring_task_demo/factory_nut.usd",
            scale=(1.0, 1.0, 1.0),
    ...
    # Collection box (below table - positioned lower to represent box under sink)
    collection_box = RigidObjectCfg(
        prim_path="{ENV_REGEX_NS}/CollectionBox",
        init_state=RigidObjectCfg.InitialStateCfg(pos=[0.0, 0.55, 0.50], rot=[1.0, 0, 0, 0]),
        spawn=UsdFileCfg(
            usd_path=f"{ISAACLAB_NUCLEUS_DIR}/Props/Mimic/nutpouring_task_demo/sorting_bin.usd",
            scale=(1.0, 1.0, 1.0),
            rigid_props=sim_utils.RigidBodyPropertiesCfg(),
        ),
    )
...
class TerminationsCfg:
    ...
    trash_item_1_dropped = DoneTerm(
        func=mdp.root_height_below_minimum,
        params={"minimum_height": 0.5, "asset_cfg": SceneEntityCfg("trash_item_1")},
    )

    trash_item_2_dropped = DoneTerm(
        func=mdp.root_height_below_minimum,
        params={"minimum_height": 0.5, "asset_cfg": SceneEntityCfg("trash_item_2")},
    )

    success = DoneTerm(func=mdp.task_done_tcr_bussing)
...
class EventCfg:
    ...
    reset_trash_item_1 = EventTerm(
        ...
        params={
            ...
            "velocity_range": {},
            "asset_cfg": SceneEntityCfg("trash_item_1"),
        },
    )

    reset_trash_item_2 = EventTerm(
        func=mdp.reset_root_state_uniform,
        mode="reset",
        params={
            "pose_range": {
            ...
            "velocity_range": {},
            "asset_cfg": SceneEntityCfg("trash_item_2"),
        },
    )


@configclass
class TcrBussingEasyGR1T2BaseEnvCfg(ManagerBasedRLEnvCfg):
    """Configuration for the GR1T2 TCR Bussing environment."""

    # Scene settings
    scene: TcrBussingEasySceneCfg = TcrBussingEasySceneCfg(num_envs=1, env_spacing=2.5, replicate_physics=True)
    # Basic settings
